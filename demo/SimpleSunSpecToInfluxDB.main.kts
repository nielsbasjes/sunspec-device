...
fun runLoop(device: SchemaDevice, writeApi: WriteApiBlocking?, databaseMeasurement: String) {
    ...
    // These are always needed as tags for the data in InfluxDB
    allFields.put("AC Current",               fieldOrFail(device["Model 101"]["A"           ]))
    allFields.put("Phase A Current",          fieldOrFail(device["Model 101"]["AphA"        ]))
    allFields.put("Phase B Current",          fieldOrFail(device["Model 101"]["AphB"        ]))
    allFields.put("Phase C Current",          fieldOrFail(device["Model 101"]["AphC"        ]))
    allFields.put("Phase Voltage AB",         fieldOrFail(device["Model 101"]["PPVphAB"     ]))
    allFields.put("Phase Voltage BC",         fieldOrFail(device["Model 101"]["PPVphBC"     ]))
    allFields.put("Phase Voltage CA",         fieldOrFail(device["Model 101"]["PPVphCA"     ]))
    allFields.put("Phase Voltage AN",         fieldOrFail(device["Model 101"]["PhVphA"      ]))
    allFields.put("Phase Voltage BN",         fieldOrFail(device["Model 101"]["PhVphB"      ]))
    allFields.put("Phase Voltage CN",         fieldOrFail(device["Model 101"]["PhVphC"      ]))
    allFields.put("AC Power",                 fieldOrFail(device["Model 101"]["W"           ]))
    allFields.put("Line Frequency",           fieldOrFail(device["Model 101"]["Hz"          ]))
    allFields.put("AC Apparent Power",        fieldOrFail(device["Model 101"]["VA"          ]))
    allFields.put("AC Reactive Power",        fieldOrFail(device["Model 101"]["VAr"         ]))
    allFields.put("AC Power Factor",          fieldOrFail(device["Model 101"]["PF"          ]))
    allFields.put("AC Energy",                fieldOrFail(device["Model 101"]["WH"          ]))
    allFields.put("DC Current",               fieldOrFail(device["Model 101"]["DCA"         ]))
    allFields.put("DC Voltage",               fieldOrFail(device["Model 101"]["DCV"         ]))
    allFields.put("DC Power",                 fieldOrFail(device["Model 101"]["DCW"         ]))
    allFields.put("Cabinet Temperature",      fieldOrFail(device["Model 101"]["TmpCab"      ]))
    allFields.put("Heat Sink Temperature",    fieldOrFail(device["Model 101"]["TmpSnk"      ]))
    allFields.put("Transformer Temperature",  fieldOrFail(device["Model 101"]["TmpTrns"     ]))
    allFields.put("Other Temperature",        fieldOrFail(device["Model 101"]["TmpOt"       ]))

    allFields.put("Module 0 Input ID",        fieldOrFail(device["Model 160"]["Module_0_ID" ]))
    allFields.put("Module 0 DC Current",      fieldOrFail(device["Model 160"]["Module_0_DCA"]))
    allFields.put("Module 0 DC Voltage",      fieldOrFail(device["Model 160"]["Module_0_DCV"]))
    allFields.put("Module 0 DC Power",        fieldOrFail(device["Model 160"]["Module_0_DCW"]))
    allFields.put("Module 1 Input ID",        fieldOrFail(device["Model 160"]["Module_1_ID" ]))
    allFields.put("Module 1 DC Current",      fieldOrFail(device["Model 160"]["Module_1_DCA"]))
    allFields.put("Module 1 DC Voltage",      fieldOrFail(device["Model 160"]["Module_1_DCV"]))
    allFields.put("Module 1 DC Power",        fieldOrFail(device["Model 160"]["Module_1_DCW"]))
